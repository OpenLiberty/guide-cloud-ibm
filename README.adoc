...
Retrieve the list of available Kubernetes zone for deploying the cluster within the IBM Cloud classic infrastructure 
[role=command]
```
...
[role=command]
```
set AVAILABLE_ZONE=[available_zone]
...
[role=command]
```
export AVAILABLE_ZONE=[available_zone]
ibmcloud ks flavor ls  --provider classic --zone $AVAILABLE_ZONE
```
--
...
[.tab_content.windows_section]
--
Replace `[selected_flavor]` with an available flavor selected from the previous command. Then use the following command to create a Kubernetes cluster.

[role=command]
```
set SELECTED_FLAVOR=[selected_flavor]
...
[role=command]
```
export SELECTED_FLAVOR=[selected_flavor]
ibmcloud ks cluster create classic --name guide-cluster --flavor $SELECTED_FLAVOR  --zone $AVAILABLE_ZONE
```
--
