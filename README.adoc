...
TODO: Equivalent windows command
...
```
helm install --name name-app \
    --set image.repository=registry.ng.bluemix.net/[your-namespace]/name:1.0-SNAPSHOT \
    --set image.tag=1.0-SNAPSHOT \
    --set service.port=9080 \
    --set service.targetPort=9080 \
    --set ssl.enabled=false \
...
```
helm install --name ping-app \
    --set image.repository=registry.ng.bluemix.net/[your-namespace]/ping:1.0-SNAPSHOT \
    --set image.tag=1.0-SNAPSHOT \
    --set service.port=9080 \
    --set service.targetPort=9080 \
    --set ssl.enabled=false \
...
Run the integration tests against the ICP cluster:

```
mvn verify \
    -Ddockerfile.skip=true \
    -Dcluster.ip=`kubectl get nodes -l proxy=true -o jsonpath="{.items[0].status.addresses[?(@.type==\"Hostname\")].address}"` \
