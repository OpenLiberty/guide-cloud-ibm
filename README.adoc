...
The namespace must be unique within IBM Cloud, so choose something relevant that you'll 
remember for the duration of the guide.
...
When tagging and pushing your images, remeber to subtitute `[your-namespace]` for the namespace you previously created.
...
[role=command]
```
curl http://[ip-address]:[ping-node-port]/api/ping/name-app-ibm-open-libert
```
