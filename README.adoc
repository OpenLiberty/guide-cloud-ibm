...
After the script has finished running, login to IBM Cloud by using the `ibmcloud` command line. When you are prompted to select a region, choose `us-south`. Creating a free cluster is limited to specific regions, choosing `us-south` will allow you to create a free cluster.
...
Pushing the images to a registry will allow the cluster to create pods using your Docker images. Since it's a private repository, only users with access to your IBM Cloud account will have access to these images.
...
Remeber to subtitute `[your-namespace]` for the namespace you previously created.

[role=command]
```
docker tag name:1.0-SNAPSHOT us-south.icr.io/[your-namespace]/name:1.0-SNAPSHOT
docker tag ping:1.0-SNAPSHOT us-south.icr.io/[your-namespace]/ping:1.0-SNAPSHOT
```

// Pushing images
Finally, push your images to the registry. Remeber to subtitute `[your-namespace]` for the 
namespace you previously created.

[role=command]
```
docker push us-south.icr.io/[your-namespace]/name:1.0-SNAPSHOT
docker push us-south.icr.io/[your-namespace]/ping:1.0-SNAPSHOT
```
...
helm install --name name-app \
    --set image.repository=us-south.icr.io/[your-namespace]/name \
...
helm install --name ping-app \
    --set image.repository=us-south.icr.io/[your-namespace]/ping \
    --set image.tag=1.0-SNAPSHOT \
...
since we do not specify the `nodePort`, the ports will be randomized so we must obtain 
...
[role=command]
```
kubectl get service name-service -o jsonpath="{.spec.ports[0].nodePort}"
```
...
[role=command]
```
kubectl get service ping-service -o jsonpath="{.spec.ports[0].nodePort}"
```
...
Make a request to the `name` microservice substituting in `ip-address` and `name-node-port` where appropriate.

[role=command]
```
curl http://[ip-address]:[name-node-port]/api/name
```

Make a request to the `ping` microservice substituting in `ip-address` and `ping-node-port` where appropriate.

[role=command]
```
curl http://[ip-address]:[ping-node-port]/api/ping/name-service
```
...
[role=command]
```
mvn verify -Ddockerfile.skip=true -Dcluster.ip=[ip-address] -Dname.node.port=[name-node-port] -Dping.node.port=[ping-node-port]
```
...
[role=command]
```
docker tag name:2.0-SNAPSHOT us-south.icr.io/[your-namespace]/name:2.0-SNAPSHOT
```
...
[role=command]
```
docker push us-south.icr.io/[your-namespace]/name:2.0-SNAPSHOT
```
