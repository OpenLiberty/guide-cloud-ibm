...
To create a {kube} cluster you need appropriate permission. Ensure that you have `Administrator` access to IBM Cloud Kubernetes Service. Check by navigating to `Manage > Access (IAM) > Users > [Your Username] > Access Policies`. Ensure that `Administrator` is listed as a policy.
...
First, add the `ibm-charts` repository to your local list of helm repositories. Adding the repository allows you to use the `ibm-open-liberty` chart.
...
If you don't know the IP address and ports you can get them using the following commands.
