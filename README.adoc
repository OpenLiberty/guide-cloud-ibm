...
hosted {kube} cluster that you can deploy your microservices to. 
...
deploys and manages them as releases in your cluster. These Helm releases are generated using
images specified when running the `helm install` command. You will use IBM Cloud
Container Registry as a private place to store the images that Helm will use when deploying 
its releases.
...
Pushing the images to a registry allow the cluster to create pods using your container images. 
...
Use the plugin again to login to the container registry.
...
A private cloud may be a more attractive option to those who must run their services on 
...
that you will notice are in the setup process and how you tag your container images.
...
as the container registry to push to and it specifies `default` as the namespace your image's 
