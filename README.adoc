...
Replace `[available_zone]` with an available zone selected from the previous command. Use the following command to retrieve the list of the available flavors of Kubernetes worker nodes for a zone.
[role=command]
```
...
Replace `[selected_flavor]` with an available flavor selected from the previous command. Use the following command to create a Kubernetes cluster.
[role=command]
```
