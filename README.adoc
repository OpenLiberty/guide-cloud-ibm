...
microservice returns the Java virtual machine (JVM) system properties of the running container. 
...
that are useful for interacting with IBM Cloud, including the `ibmcloud`, `kubectl`, `helm`, 
...
(see https://openliberty.io/{page-permalink}#installing-command-line-tools-for-ibm-cloud[Installing command line tools for IBM Cloud]).
...
Use the plug-in again to login to the container registry.
...
Remember to replace `[your-namespace]` for the namespace you created earlier in the guide.
...
// Deploy to IKS
Use Helm to deploy the `system` microservice. Remember to replace `[your-namespace]` for the namespace you created earlier in the guide.
...
Use Helm to deploy the `inventory` microservice. Remember to replace `[your-namespace]` for the namespace you created earlier in the guide.
...
the new version of the microservice. You can redeploy a microservice using Helm to upgrade 
the release.
