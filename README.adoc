...
limited to specific regions, choosing `us-south` allows you to create a free cluster. Note 
that if you are using a federated user ID, you will have to use the `--sso` flag to get a 
one time code for single sign-on.
[role=command]
```
ibmcloud login
```
