...
hosted {kube} cluster that you can deploy your microservices to. You will use IKS 
with IBM Cloud Container Registry.

You will use Helm to deploy these microservices using Open Liberty Helm charts. Helm is 
a package manager for {kube}. It uses templates to generate {kube} yaml files and then 
...
https://www.ibm.com/cloud/private[IBM Cloud Private^] (ICP) is a {kube} platform. 
