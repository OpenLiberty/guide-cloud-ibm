...
hosted {kube} cluster that you can deploy your microservices to. You will use it in conjunction 
...
To create a {kube} cluster you need appropriate permission. Ensure that you have `Administrator` access to IBM Cloud Kubernetes Service. Navigate to the https://cloud.ibm.com/[IBM Cloud Dashboard^]. Then navigate to `Manage > Access (IAM) > Users > [Your Username] > Access Policies` and ensure that `Administrator` is listed as a policy for all resources in the account or for the Kubernetes service.
...
After the script is finished running, login to IBM Cloud by using the `ibmcloud` command line. When you are prompted to select a region, choose `us-south`. Creating a free cluster is limited to specific regions, choosing `us-south` allows you to create a free cluster.
...
[role=command]
```
docker tag system:1.0-SNAPSHOT us-south.icr.io/[your-namespace]/system:1.0-SNAPSHOT
docker tag inventory:1.0-SNAPSHOT us-south.icr.io/[your-namespace]/inventory:1.0-SNAPSHOT
```
...
[role=command]
```
docker push us-south.icr.io/[your-namespace]/system:1.0-SNAPSHOT
docker push us-south.icr.io/[your-namespace]/inventory:1.0-SNAPSHOT
```
...
helm install --name system-app \
    --set image.repository=us-south.icr.io/[your-namespace]/system \
...
helm install --name inventory-app \
    --set image.repository=us-south.icr.io/[your-namespace]/inventory \
    --set image.tag=1.0-SNAPSHOT \
...
since we do not specify the `nodePort`, the ports are randomized so we must obtain 
the ports before making requests to our services. We must also obtain the public IP address 
...
kube-hou02-pad1539f4f9ee912ca08c6b187c2daa5f9-w1   172.173.65.24   10.77.198.71   free           normal   Ready    hou02   1.10.11_1538*
----
...
Make a request to the `system` microservice substituting in `ip-address` and `system-node-port` where appropriate.

[role=command]
```
curl http://[ip-address]:[system-node-port]/system/properties 
```

Make a request to the `inventory` microservice substituting in `ip-address` and `inventory-node-port` where appropriate.

[role=command]
```
curl http://[ip-address]:[inventory-node-port]/inventory/systems/system-service
```
...
| system.kube.service | Name of the {kube} Service wrapping the `name` pods, `system-service` by default.
...
Optionally, you may want to learn how to re-deploy a microservice. You can re-deploy a 
microservice using Helm to upgrade the release.
...
[role=command]
```
docker tag system:2.0-SNAPSHOT us-south.icr.io/[your-namespace]/system:2.0-SNAPSHOT
```
...
[role=command]
```
docker push us-south.icr.io/[your-namespace]/system:2.0-SNAPSHOT
```
...
https://www.ibm.com/cloud/private[IBM Cloud Private^] (ICP) is a {kube} platform. A private cloud may be a more attractive option to those who must run their services running on infrastructure that is controlled by them. While the operations team will still need to maintain this infrastructure, a private cloud still provides benefit to developers so they don't need to think about infrastructure. {kube} acts as an abstraction of the infrastructure so the developers can request what they need and it is the responsibility of the infrastructure's maintainers to provide the necessary resources.

Deploying microservices to ICP is a similar process as deploying to IKS. The main differences that you will notice are in the setup process and how you tag your Docker images.
